...
class TestDateTokenizer(TestCase):
    ...
    def test_merge_tokens(self):
        ...
        self.assertEqual('At', merged[0].captures['extra_tokens'][0])
    ...
    def test_compare_date_string(self):
        ...
        tokens = dtok.tokenize_string(text)
        merged = dtok.merge_tokens(tokens)
        pattern_start = '#'.join(['t t', 't t', ' of ', 'on of t', ' to T',
                                  ' of ', ' at 22.08', ' of t', 't of t', ' by ',
                                  ' July 1, 1994 to June 30, 1995.\n               '])
        merged_start = '#'.join([m.match_str for m in merged])
