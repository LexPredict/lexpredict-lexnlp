...
def get_ratios(text, return_sources=False, float_digits=4):
    result = []
    for source_text, ratio_1_text, ratio_2_text in RATIO_PTN_RE.findall(text.lower()):
        amount_1 = get_amounts(ratio_1_text, float_digits=float_digits)
        amount_2 = get_amounts(ratio_2_text, float_digits=float_digits)
