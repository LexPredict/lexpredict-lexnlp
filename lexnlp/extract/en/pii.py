...
def get_ssns(text, return_sources=False):
    """
    ...
    for match in RE_SSN.finditer(text):
        ...
        if return_sources:
            yield ssn, match.span(0)
        else:
            yield ssn


def get_us_phones(text, return_sources=False):
    """
    Find possible SSN references in the text.
    :param text:
    :param return_sources:
    ...
    for match in RE_US_PHONE.finditer(text):
        ...
        if return_sources:
            yield phone, match.span(0)
        else:
            yield phone


def get_pii(text, return_sources=False):
    """
