...
def get_raw_dates(text, strict=False, base_date=None, return_source=False):
    """
    ...
    # Classify and return potential dates
    dates = []
...
def get_dates(text, strict=False, base_date=None, return_source=False, threshold=0.50):
    """
    ...
    """
    # Get raw dates
    raw_date_results = get_raw_dates(text, strict=strict, base_date=base_date, return_source=True)
    date_results = []
    ...
    for raw_date in raw_date_results:
        ...
        if date_score[0, 1] >= threshold:
            if return_source:
                date_results.append((raw_date[0], raw_date[1]))
            else:
                date_results.append(raw_date[0])
...
def build_date_model(input_examples, output_file, verbose=True):
    ...
    for example in input_examples:
        # Get raw dates
        date_results = get_raw_dates(example[0], strict=False, return_source=True)
