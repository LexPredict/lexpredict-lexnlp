...
def get_definitions(text=None, sentences=None, return_sources=False, decode_unicode=True):
    """
    ...
    # Check bad argument combos
    if (text is None) and (sentences is None):
        raise ValueError("Must provide at least one of text or sentences.")
    elif text is not None and sentences is not None:
        raise ValueError("Must provide only one of text or sentences.")
    ...
    for sentence in sentences:
        ...
        for term in result:
            if len(get_tokens(term)) <= MAX_TERM_TOKENS:
                if return_sources:
                    yield (term, sentence)
                else:
                    yield term
